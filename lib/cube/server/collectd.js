exports.putter = function(putter) {
  ...
  flushInterval = setInterval(function() {
    ...
    queue.forEach(function(value) {
      ...
      if (value.time > latest) {
        ...
        if (value.plugin_instance) event = event[value.plugin + '_' + value.plugin_instance] || (event[value.plugin + '_' + value.plugin_instance] = {});
        if (value.type != value.plugin) event = event[value.type] || (event[value.type] = {});
        if (value.type_instance != '') event = event[value.type_instance] || (event[value.type_instance] = {});
        //if (value.values.length == 1) store(value, 0, event, value.type_instance);
        //else 
        value.values.forEach(function(d, i) { store(value, i, event, value.dsnames[i]); });
      }
    });
  ...
  return function(request, response) {
    ...
    request.on("end", function() {
      ...
      //parse the querystring to identify the values we need to merge
      merge = require('url').parse(request.url, true);
      	
      JSON.parse(content).forEach(function(value) {
      	value.time = Math.round(value.time * 1000);
      	
      	//do the value merging
      	for( var i in merge.query) {
      		value[i] = merge.query[i];
      	}
