var reduces = module.exports = {
  ...
  percentile: function(values, p) {
    if (!p) { p = "0.95"; }
    // TODO modify query expression grammar in
    // lib/cube/server/metric-expression.peg to allow additional arguments
    // to reduce functions
    return quantile(values.sort(ascending), parseFloat(p));
  }
...
reduces.median.empty = 0;
reduces.percentile.empty = 0;
